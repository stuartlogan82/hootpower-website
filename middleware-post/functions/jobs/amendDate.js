exports.handler =  async function(context, event, callback) {
   ...
   let updateObject = {};
   updateObject.Id = id;
   updateObject.jobDate__c = date;
   

   let job = await jobUpdate(updateObject);
   if(job === 'failed'){
       //The insert failed
...
function jobUpdate(inp){
    return new Promise(function(resolve, reject){
        console.log('Here: '+JSON.stringify(inp));
        conn.sobject("Job__c").update(inp, function(err, ret) {
            if (err || !ret.success) { 
              console.error('There has been a failure in the SFDC task update request, here is the error: '+err);
              resolve('failed'); 
            }
